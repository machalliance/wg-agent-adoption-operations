...
| 4. Live operations | Monitoring Levels, and a Monitoring and Incident Preparedness Record per agent | Keeps those evals running against the live agent, and says what happens when one fails |
...
So the draft says what it means. It writes the risk that is left where other frameworks write residual risk, how far the damage spreads for blast radius, an agent login for a non-human identity, and a risk classification for a risk tier. The [glossary](https://github.com/machalliance/wg-agent-adoption-operations/blob/main/deliverables/agent-governance-assurance-framework/glossary.md) maps each plain term to the formal one other frameworks use, so you can still take this into a room with an auditor.
